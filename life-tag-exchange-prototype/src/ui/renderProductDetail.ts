...
export function renderProductDetail(app: AppRuntime): string {
  const product = app.state.dayState.productCandidates.find(
    (candidate) => candidate.id === app.state.dayState.selectedProductId,
  );
  ...
  if (!product) {
    return `
      <section class="panel" aria-label="商品详情">
        <h2>商品详情</h2>
        <p>当前没有选中商品。点击商品候选可查看 P0-3 安全详情。</p>
      </section>
    `;
  }
  ...
  return `
    <section class="panel" aria-label="商品详情">
      ...
      <p class="hint-text">未揭示隐藏标签与暗风险详情不会在 P0-3 泄露。</p>
    </section>
  `;
}
